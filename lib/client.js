window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS_ID = "gemini-translate/window.css.v5";
    var CSS = [
    ...
    function TranslateSendButton(props) {
      ...
      var onClick = function () {
        ...
        var draft = "";
        if (props && props.useInput) {
          try { draft = props.useInput(function (s) { return s.draft; }) || ""; } catch (_) {}
        }
        ...
        setBusy(true);
        setTitle("正在译成英文…");
        streamTranslate(draft, "en", function (english) {
          inputActions.setDraft(english);
        }).then(function (english) {
          inputActions.setDraft(english || draft);
          if (typeof inputActions.submit === "function") inputActions.submit();
          setTitle("");
        }).catch(function (err) {
          setTitle(err && err.message ? err.message : String(err));
      ...
      return h("button", {
        type: "button",
        className: "gtw-chip",
        title: title || "把输入框里的中文译成英文，然后发给当前模型",
        disabled: busy,
        onClick: onClick,
      }, busy ? "翻译中…" : "译发");
    }
    ...
    function SettingsCard() {
      ...
      var save = function () {
        ...
        api("/config", { method: "PUT", body: JSON.stringify(patch) }).then(function (data) {
          setCfg(Object.assign({}, cfg, data, { apiKey: "" }));
          setStatus("已保存");
        }).catch(function (err) {
      ...
      return h("li", { className: "gtw-pcard" },
        h("button", {
          ...
          h("span", { className: "gtw-pcard-text" },
            ...
            h("span", { className: "gtw-pcard-desc" }, "输入框「译发」把中文译成英文再发送")
          ),
