    ...
    function SettingsCard() {
      ...
      var probe = function () {
        ...
        api("/models", { method: "POST", body: JSON.stringify(body) }).then(function (data) {
          ...
          setModels(list);
          setStatus(list.length ? ("探测到 " + list.length + " 个模型，点选后会收起") : "没有探测到模型");
        }).catch(function (err) {
          setModels([]);
          setStatus(err && err.message ? err.message : String(err));
      ...
      var pick = function (id) {
        var next = Object.assign({}, cfg, { model: id });
        setCfg(next);
        setModels([]);
        setBusy(true);
        var patch = { model: id, baseUrl: next.baseUrl, apiType: next.apiType };
        if (next.apiKey && next.apiKey.trim()) patch.apiKey = next.apiKey.trim();
        api("/config", { method: "PUT", body: JSON.stringify(patch) }).then(function (data) {
          setCfg(function (prev) { return Object.assign({}, prev, data, { apiKey: "", model: id }); });
          setStatus("已选择 " + id);
          setOpen(false);
        }).catch(function (err) {
          setStatus(err && err.message ? err.message : String(err));
      ...
      return h("li", { className: "gtw-pcard" },
        ...
        open ? h("div", { className: "gtw-pcard-body" },
          ...
          models.length ? h("div", { className: "gtw-models" },
            models.map(function (id) {
              return h("button", {
                key: id,
                type: "button",
                className: id === cfg.model ? "gtw-model gtw-model-on" : "gtw-model",
                onClick: function (ev) { ev.stopPropagation(); pick(id); },
              }, id);
            })
          ) : null,
